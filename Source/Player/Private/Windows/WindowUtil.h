...
void window_util::CenterWindow(HWND hWnd, HWND hDlg)
{
    ...
    const uint32_t HEIGHT = MAX(WINDOW_HEIGHT, DIALOG_HEIGHT);

    if (HEIGHT == WINDOW_HEIGHT)
    {
        MoveWindow(hWnd, CENTER_SCREEN_X - WIDTH / 2, CENTER_SCREEN_Y - HEIGHT / 2, WINDOW_WIDTH, WINDOW_HEIGHT, TRUE);
        MoveWindow(hDlg, CENTER_SCREEN_X - WIDTH / 2 + WINDOW_WIDTH, CENTER_SCREEN_Y - HEIGHT / 2 + DIALOG_HEIGHT / 3, DIALOG_WIDTH, DIALOG_HEIGHT, TRUE);
    }
    else
    {
        MoveWindow(hWnd, CENTER_SCREEN_X - WIDTH / 2, CENTER_SCREEN_Y - HEIGHT / 2 + WINDOW_HEIGHT / 3, WINDOW_WIDTH, WINDOW_HEIGHT, TRUE);
